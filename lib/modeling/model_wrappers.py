...
# from block_opts.tgcn import make_temporal_reasoning
# from ops.basic_ops_wrap import TemporalAggreModel
...
class VideoModelWrapper(nn.Module):
    def __init__(self, num_class,
                 ...
                 dropout=0.8,
                 crop_num=1, 
                 partial_bn=True, fc_sche=False, custom_flg=False, pretrained=False, pretrain_path=None):
    ...
    def forward(self, x):
        ...
        # print('=================================')
        # dataset input shape (B, CxT, H, W)
        # 2d (BxT, C, H, W)
        # 3d (B, C, T, H, W)
        if self.backbone_type == '2D':
            x = x.view((-1, sample_len) + x.shape[-2:])
        elif self.backbone_type == '3D':
            x = x.view((-1, sample_len, self.clip_length) + x.shape[-2:])  # 3d model have temporal pooling here

        base_out = self.base_model(x)
        # output 2d (BxT, num_classes)
        # output 3d (B, T, num_classes) excep temporal pooling
    ...
    def _prepare_video_model(self, num_class):
        ...
        if self.custom_flg:
            ...
            print('todo temporal reasoning')

        return feature_dim

    def _prepare_base_model(self, backbone_name):
        
        #self.base_model = backbones.build_backbone(self.cfg, backbone)
        if self.backbone_type == '2D':
            self.base_model = getattr(backbone.backbone_2d, backbone_name)()
        elif self.backbone_type == '2D':
