...
if __name__ == "__main__":
    # not to tensor
    def save_clips(file_name, clips):
        # only for one kind (no mormalize and to_tensor)
        # clips = [img.numpy.astype('uint8') for img in clips]
        for i, img in enumerate(clips):
            cv2.imwrite(file_name+'_'+str(i)+'.jpg', img)

    import cv2
    clips = [cv2.imread('example.jpg') for _ in range(3)]

    # # random resize crop
    # trans = VideoRandomResizedCrop(224)
    # trans_clips = trans(clips)
    # save_clips('v_random_resized_crop', trans_clips)

    # # random crop
    # trans = VideoRandomCrop(224)
    # trans_clips = trans(clips)
    # save_clips('v_random_crop', trans_clips)

    # # center crop
    # trans = VideoCenterCrop(224)
    # trans_clips = trans(clips)
    # save_clips('v_center_crop', trans_clips)

    # # random horizontal flip
    # trans = VideoRandomHorizontalFlip(p=0.7)
    # trans_clips = trans(clips)
    # save_clips('v_random_horizontal_flip', trans_clips)

    # # random rotate
    # trans = VideoRotate(limit=90)
    # trans_clips = trans(clips)
    # save_clips('v_random_rotate', trans_clips)

    # # add gaussian noise
    # trans = VideoGaussianNoise()
    # trans_clips = trans(clips)
    # save_clips('v_add_gaussian_noise', trans_clips)

    # # gaussian blur
    # trans = VideoGaussianBlur()
    # trans_clips = trans(clips)
    # save_clips('v_gaussian_blur', trans_clips)

    # color jitter
    # trans = VideoRandomColorJitter()
    # trans_clips = trans(clips)
    # save_clips('v_random_colorjitter', trans_clips)