...
class BaseDataset(data.Dataset):
    def __init__(self, root_path, list_file,
                 video_length=3, new_length=1, sample_type='uniform', modality='RGB',
    ...
    def _get_dense_indices(self, record, step=4):
        expanded_sample_length = self.video_length * step  # in order to drop every other frame, cover several seconds (duration = expanded_sample_length/fps)
        ...
        offsets =np.array([int(v) for v in offsets])  # images are 1-indexed
    ...
    def _get_adver_indices(self, record, nsamples):
        ...
        if average_duration > 0:
            ...
            #     ticks.extend(randint(average_duration, size=self.video_length))
            ...
            #offsets = [np.sort(i) + 1 for i in ticks]
        else:
            offsets = []
            for i in range(nsamples):
                offsets.extend(np.zeros((self.video_length, ))+1)
            #offsets = [np.zeros((self.video_length,))+1 for i in range(nsamples)]
        return offsets
    ...
    def __getitem__(self, index):
        ...
        if not self.test_mode:
            if self.use_adver:
                ...
                segment_indices = self._get_dense_indices(record)
        else:
